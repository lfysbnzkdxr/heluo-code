...
- **根因修复，直接覆盖**：发现缺陷时定位根因，用正确逻辑整体替换，不打补丁；禁止以「临时跳过/绕过/开关」回避问题，调试开关与打点用完即删，不留 TEMP-DEBUG 残留
- **肯定式描述**：注释与文档用肯定句描述事实与决策（如「stdio 采用继承直通」「X 经实测可用」），禁止写否定的否定（如「不启用 X」「无 Y 问题」）——否定式表述遮蔽真实设计意图，读代码的人无法判断是决策还是遗留
