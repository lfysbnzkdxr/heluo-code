...
**编码基线（v1）**：文件默认以 **UTF-8** 读写。`read_file` 遇 BOM 或无效 UTF-8 字节序列时，尽力按原字节输出并附警告（不静默损坏）；`write_file` 始终以 UTF-8 写入（结果中注明编码）。自动编码探测（如识别 GBK/UTF-16 并保留原编码写回）列为 P6 增强——避免非 UTF-8 项目文件被写入破坏。
